...
(defn surrounding-coordinates [[x0 y0 z0 w0]]
  (for [x [(dec x0) x0 (inc x0)]
        y [(dec y0) y0 (inc y0)]
        z [(dec z0) z0 (inc z0)]
        w [(dec w0) w0 (inc w0)]
        :when (or (not= x x0) (not= y y0) (not= z z0) (not= w w0))]
    [x y z w]))
...
;; TESTS
(defn test-solution [] (= (solve example-input) 848))
